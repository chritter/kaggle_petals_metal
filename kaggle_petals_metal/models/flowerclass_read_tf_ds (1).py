...
def load_dataset(
    filenames, IMAGE_SIZE, RESIZE, labeled=True, ordered=False, with_id=False
):
    ...
    read_labeled_tfrecord2 = partial(
        read_labeled_tfrecord, IMAGE_SIZE=IMAGE_SIZE, RESIZE=RESIZE, with_id=with_id
    )
    read_unlabeled_tfrecord2 = partial(
        read_unlabeled_tfrecord, IMAGE_SIZE=IMAGE_SIZE, RESIZE=RESIZE
    )
    ...
    dataset = dataset.map(
        read_labeled_tfrecord2 if labeled else read_unlabeled_tfrecord2,
...
def get_training_dataset(TRAINING_FILENAMES, BATCH_SIZE, IMAGE_SIZE, RESIZE, with_id):
    dataset = load_dataset(
        TRAINING_FILENAMES, IMAGE_SIZE, RESIZE, labeled=True, with_id=with_id
    )
...
def get_validation_dataset(
    VALIDATION_FILENAMES, BATCH_SIZE, IMAGE_SIZE, RESIZE, with_id, ordered=False
):
    dataset = load_dataset(
...
def get_test_dataset(TEST_FILENAMES, BATCH_SIZE, IMAGE_SIZE, RESIZE, ordered=False):
    dataset = load_dataset(
        TEST_FILENAMES, IMAGE_SIZE, RESIZE, labeled=False, ordered=ordered
    )
    dataset = dataset.batch(BATCH_SIZE)
    dataset = dataset.prefetch(AUTO)
    return dataset
...
def get_datasets(BATCH_SIZE, IMAGE_SIZE, RESIZE, tpu=False, with_id=False):
    ...
    ds_train = get_training_dataset(
        TRAINING_FILENAMES, BATCH_SIZE, IMAGE_SIZE, RESIZE, with_id=with_id
    )
    ds_valid = get_validation_dataset(
        VALIDATION_FILENAMES, BATCH_SIZE, IMAGE_SIZE, RESIZE, with_id=with_id
    )
